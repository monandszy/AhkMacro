...
global MyLogFile:="C:\Users\Name\..My\VSCode\AutoHotKey\log.log"
LogFile:="C:\Users\Name\Desktop\Temp\Record.ahk"
RecordFile:="C:\Users\Name\Desktop\Temp\Record.ahk"
UsedKeys:="F1,F2,F3,F4,F5,F6,F8,F9,F10"
...
s:="[F1]Rec (Scr),[F2]Rec (Win),"
  . "[F3]Stop,[F4]Play,[F5]Edit,[F6]Pause,[F7]Exit,[F9]Options "
...
;~ F7::
; Adjust to stop other script execution
F3::
Stop:
Suspend, Permit
if Recording
{
  ...
  {
    s:="playspeed:=" playspeed " `nLoop, 1`n{`nSetTitleMatchMode, 2"
      . "`nCoordMode, Mouse, " Coord "`n"
    For k,v in LogArr
      s.="`n" v 
    ;~ s.="`nSleep, 1000`n`n}`n"
    s.="`n  Sleep, 100  //playSpeed `n}"
    s:=RegExReplace(s,"\R","`n")
    FileDelete, %RecordFile%
    FileAppend, %s%, %RecordFile%
    s:=""
  }
...
{
  isPaused:=!isPaused
  WinGet, titles, List, %Play_Title%
  Loop, %titles%
    if WinExist("ahk_id " titles%A_Index%)!=A_ScriptHwnd
...
SetHotkey(f:=0) {
  ...
  Loop, 254
  {
    ; FileAppend, "k(GKN):["%A_Index%"]`n", %MyLogFile%,
    k:=GetKeyName(vk:=Format("vk{:X}", A_Index))
    ; FileAppend, "k(GKN):["%k%"]`n", %MyLogFile%,
    if k not in ,Control,Alt,Shift,%UsedKeys% 
    {
      ; Hotkey, ~*%vk%, LogKey, %f% UseErrorLevel
      Hotkey, % "~*" vk " Up", CustomLogKey
      Hotkey, % "~*" vk, CustomLogKey
    }
  }
    ...
    . "|PgDn|Left|Right|Up|Down|Delete|Insert", "|")
  {
    ...
    if k not in ,Control,Alt,Shift,%UsedKeys% 
    {
      ...
      Hotkey, % "~*" sc " Up", CustomLogKey
      Hotkey, % "~*" sc , CustomLogKey
    }
  }
...
CustomLogKey() {
  ...
  else if(k in LButton,RButton,MButton && keyState != " Up")
